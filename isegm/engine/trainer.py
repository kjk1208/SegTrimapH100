...
class ISTrainer(object):
    ...
    def training(self, epoch):
        ...
        for i, batch_data in enumerate(tbar):
            ...
            # kjk
            losses_logging = {k: (v if isinstance(v, torch.Tensor) else torch.tensor(v, device=self.device)) 
                  for k, v in losses_logging.items()}
            # kjk
    ...
    def validation(self, epoch):
        ...
        for i, batch_data in enumerate(tbar):
            ...
            # kjk
            batch_losses_logging = {k: (v if isinstance(v, torch.Tensor) else torch.tensor(v, device=self.device)) 
                        for k, v in batch_losses_logging.items()}
            # kjk
