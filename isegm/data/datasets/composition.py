...
class COMPOSITIONTrimapDataset(ISDataset):
    def __init__(self, dataset_path, split='train', **kwargs):
        ...
        self.dataset_samples = sorted([p.stem for p in self.image_dir.glob('*.png')])

    def get_sample(self, index) -> DSample:
        sample_id = self.dataset_samples[index]

        # 1. Load image
        image = cv2.imread(str(self.image_dir / f'{sample_id}.png'))
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

        # 2. Load and binarize seg_mask from alpha matte
        alpha = cv2.imread(str(self.mask_dir / f'{sample_id}.jpg'), cv2.IMREAD_GRAYSCALE)
        seg_mask = (alpha >= 30).astype(np.uint8)  # [H, W]
        seg_mask = seg_mask[:, :, None]             # [H, W, 1]

        # 3. Load GT trimap
        trimap = cv2.imread(str(self.trimap_dir / f'{sample_id}.jpg'), cv2.IMREAD_GRAYSCALE)
